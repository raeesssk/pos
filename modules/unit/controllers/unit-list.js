// import admin
angular.module('unit').controller('unitListCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route) {
  ...
  $scope.filteredTodos = [];
    ...
    $scope.unitList = [];
    $scope.loading1 = 0;
$scope.apiURL = $rootScope.baseURL+'/unit';
	      ...
	      category.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.unitList.length)
                          $scope.filterUser = $scope.unitList.length;
                      $scope.filteredTodos = $scope.unitList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.unitList;
              $scope.loading1 = 1;
              // $scope.$apply(); 
	    })
	    .error(function(data) 
    ...
    $scope.getSearch = function () {
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].um_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    || String($scope.obj_Main[i].um_name).toLowerCase().includes($scope.searchtext.toLowerCase())
                ) {
                    $scope.unitList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.unitList = [];
            $scope.unitList = $scope.obj_Main;
        }
