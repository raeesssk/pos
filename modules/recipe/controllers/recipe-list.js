...
angular.module('recipe').controller('recipeListCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route) {
  ...
  $scope.filteredTodos = [];
    ...
    $scope.obj_Main = [];
    $scope.recipeList = [];
    $scope.recipeListcount=0;
    $scope.limit={};
    $scope.loading1 = 0;
$scope.apiURL = $rootScope.baseURL+'/recipe/recipe/total';
      ...
      $http({
	    ...
	    .success(function(recipe)
	    {
	      recipe.forEach(function (value, key) {
                  $scope.recipeListcount=value.total;
              });
   ...
   //Pagination Function
      $scope.resetpagination = function () {
        ...
        if ($scope.filterUser >= $scope.recipeListcount)
            $scope.filterUser = $scope.recipeListcount;
              ...
              $http({
                method: 'POST',
                url: $rootScope.baseURL+'/recipe/recipe/limit',
                data: $scope.limit,
                headers: {'Content-Type': 'application/json',
                          'Authorization' :'Bearer '+localStorage.getItem("pos_admin_access_token")}
              })
