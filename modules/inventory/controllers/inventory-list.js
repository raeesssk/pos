...
angular.module('inventory').controller('inventoryListCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route) {
  ...
  $scope.filteredTodos = [];
    ...
    $scope.inventoryList = [];
    $scope.loading1 = 0;
$scope.apiURL = $rootScope.baseURL+'/inventory';
      ...
      $http({
	    ...
	    .success(function(inventory)
	    {
	      inventory.forEach(function (value, key) {
                  $scope.inventoryList.push(value);
              });
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.inventoryList.length)
                          $scope.filterUser = $scope.inventoryList.length;
                      $scope.filteredTodos = $scope.inventoryList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.inventoryList;
              $scope.loading1 = 1;
              // $scope.$apply(); 
	    })
	    .error(function(data) 
    ...
    $scope.getSearch = function () {
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].um_name).toLowerCase().includes($scope.searchtext.toLowerCase())
                    || String($scope.obj_Main[i].im_name).toLowerCase().includes($scope.searchtext.toLowerCase())
                ) {
                    $scope.inventoryList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.inventoryList = [];
            $scope.inventoryList = $scope.obj_Main;
        }
