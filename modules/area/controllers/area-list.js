// import admin
angular.module('area').controller('areaListCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route) {
  ...
  $scope.filteredTodos = [];
    ...
    $scope.areaList = [];
    $scope.loading1 = 0;
$scope.apiURL = $rootScope.baseURL+'/area';
	      ...
	      area.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.areaList.length)
                          $scope.filterUser = $scope.areaList.length;
                      $scope.filteredTodos = $scope.areaList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.areaList;
              $scope.loading1 = 1;
              // $scope.$apply(); 
	    })
	    .error(function(data) 
   ...
   //Pagination Function
    ...
    $scope.getSearch = function () {
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].am_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    || String($scope.obj_Main[i].am_name).toLowerCase().includes($scope.searchtext.toLowerCase())
                ) {
                    $scope.areaList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.areaList = [];
            $scope.areaList = $scope.obj_Main;
        }
        $scope.resetpagination();
        $scope.$apply();
    };

    $scope.deleteArea = function (am_id) {
      $scope.am_id=am_id;
    }  
    ...
    $scope.deleteConfirm = function () {
	     ...
	     $http({
	      method: 'POST',
        url: $rootScope.baseURL+'/area/delete/'+$scope.am_id,
	      headers: {'Content-Type': 'application/json',
                  'Authorization' :'Bearer '+localStorage.getItem("pos_admin_access_token")}
	    })
