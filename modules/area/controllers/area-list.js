...
angular.module('area').controller('areaListCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route) {
  ...
  $scope.filteredTodos = [];
    ...
    $scope.obj_Main = [];
    $scope.areaList = [];
    $scope.areaListcount=0;
    $scope.limit={};
    $scope.loading1 = 0;
$scope.apiURL = $rootScope.baseURL+'/area/area/total';
  ...
  var access = permission.includes(value);
   ...
   $scope.getAll = function () {
      ...
      $scope.limit.am_srm_id = localStorage.getItem("pos_admin_srm_id");
      $http({
	      method: 'POST',
	      url: $scope.apiURL,
        data:$scope.limit,
	    ...
	    .success(function(area)
	    {
	      area.forEach(function (value, key) {
                  $scope.areaListcount=value.total;
              });
   ...
   //Pagination Function
    $scope.resetpagination = function () {
        ...
        if ($scope.filterUser >= $scope.areaListcount)
            $scope.filterUser = $scope.areaListcount;
              ...
              $http({
                method: 'POST',
                url: $rootScope.baseURL+'/area/area/limit',
                data: $scope.limit,
                headers: {'Content-Type': 'application/json',
                          'Authorization' :'Bearer '+localStorage.getItem("pos_admin_access_token")}
              })
